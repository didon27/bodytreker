...
function* getFriendsActivities(data) {
  const {payload, refresh} = data;

  try {
    const response = yield call(api.activities.getActivities, payload);
    yield put(
      activitiesActions.getFriendsActivitiesSuccess(
        response.data,
        refresh,
      ),
    );
    ...
    yield put(
      activitiesActions.getFriendsActivitiesFailure(
...
export function* activitiesSaga() {
  ...
  yield takeLatest(
    activitiesConstants.GET_FRIENDS_ACTIVITIES,
